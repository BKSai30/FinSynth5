/**
 * Forecast table component displaying monthly revenue data.
 * Shows detailed breakdown of large customer, SMB, and total revenue.
 */
...
export function ForecastTable({ result, onExportExcel }: ForecastTableProps) {
  ...
  const formatMonth = (monthNumber: number) => {
    const date = new Date(2024, monthNumber - 1) // Using 2024 as base year
      ...
      <CardHeader className="pb-4">
        ...
        <div className="bg-muted/20 rounded-lg p-3 border border-border/30">
          <p className="text-sm font-medium">
            Forecast: {result.timeframe_months} Months | Total Revenue: {formatCurrency(result.summary.total_revenue)}
          </p>
        </div>
      </CardHeader>
      <CardContent>
        <div className="rounded-lg border border-border/30 overflow-hidden">
          <Table>
            ...
            <TableBody>
              {result.monthly_data.map((row, index) => (
                <TableRow key={index} className="hover:bg-muted/5 transition-colors">
                  ...
                  <TableCell className="text-right font-mono font-semibold">
                    {formatCurrency(row.total_revenue || row.revenue || 0)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
